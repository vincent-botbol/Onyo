...
(*****                                                                   *****)
(*****  Application : Demineur                                           *****)
(*****************************************************************************)

let draw_rect x0 y0 w h = 
  let (a,b) = Graphics.current_point() in 
  let x1 = x0+w and y1 = y0+h in
    Graphics.moveto x0 y0;  Graphics.lineto x0 y1;
...
let draw_num_cc wcf i j n =
  ...
  draw_box wcf.cc_bcf ;
  if n<>0 then 
    draw_string_in_box Center (string_of_int n) wcf.cc_bcf Graphics.white 
...
let open_wcf wcf = 
  Js.Node.append
    (Js.get_element_by_id "graphic_box")
